 ...
 */
class RutinasComunidadAdapter(val clickListener: RutinaModelListener): ListAdapter<RutinaModel, RutinasComunidadAdapter.ViewHolder> (RutinasComunidadDiffCallback()){
    ...
    class ViewHolder private constructor(val binding: ListItemRoutinesBinding): RecyclerView.ViewHolder(binding.root){
        fun bind(item: RutinaModel, clickListener: RutinaModelListener){
        ...
        companion object {
            fun from(parent: ViewGroup):ViewHolder{
                ...
                val binding = ListItemRoutinesBinding.inflate(layoutInflater,parent,false)
...
class RutinasComunidadDiffCallback: DiffUtil.ItemCallback<RutinaModel>(){
...
class RutinaModelListener(val clickListener: (routineId: String) -> Unit){
